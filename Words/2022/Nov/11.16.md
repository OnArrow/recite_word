...
10. post **[pəʊst]** `n` 帖子；邮递；邮件；柱子，杆子，桩 `v` 张贴（标志、通知等）；公布信息，发帖子 `prep` 在...之后
    ```
...
20. flat **[flæt]** `n` 公寓 `adj` 水平的；平坦的 `adv` 水平地；平整地 `v` 住公寓
    ```
...
22. fight **[faɪt]** `v` 与...作斗争；打架；吵架 `n` 斗争；打斗
    ```
...
38. card **[kɑːd]** `n` 明信片；卡片；银行卡；身份证 `v` 要求出示身份证
    ```
