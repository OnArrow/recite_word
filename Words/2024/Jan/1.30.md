...
5. schedule **[ˈʃedjuːl]** `n` 日程安排；计划（表）；时间表
    ```
...
10. transparent **[trænsˈpærənt]** `adj` 透明的，清澈的
    ```
...
13. interval **[ˈɪntəv(ə)l]** `n` 间隔，间隙；区间
    ```
    The process is repeated after a short interval of time
    每间隔一小段时间后程序被执行
    ```

14. crown **[kraʊn]** `n` 皇冠 `v` 立...为王
    ```
