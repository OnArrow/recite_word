...
15. pedal **[ˈped(ə)l]** `n` 脚踏板 `v` 踩（自行车等的）踏板
    ```
...
19. unaware **[ˌʌnəˈweə(r)]** `adj` 不知道的；未察觉的
    ```
