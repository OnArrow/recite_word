...
11. fancy **[ˈfænsi]** `v` （表示惊奇或震惊）真想不到，竟然
    ```
...
15. vacant **[ˈveɪkənt]** `adj` （地方）空着的；（职位）空缺的
    ```
    There are many vacant seats in the carriage.
    车厢里空得很
    ```

16. temple **[ˈtemp(ə)l]** `n` 庙宇，寺院
    ```
    The temple is now inaccessible to the public.
    这个寺庙现在不对外开放
    ```

17. prove **[pruːv]** `v` 显示出，证明是
    ```
...
19. survive **[səˈvaɪv]** `v` (经历事故，战争或疾病后)活下来，幸存
    ```
    However， not one is known to survive today.
    然而，如今没有一个人幸存下来
    ```

20. normally **[ˈnɔːməli]** `adv` 通常地
    ```
